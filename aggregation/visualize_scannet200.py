    ...
    def singleviz(self, agnostic_path, specific = False, vocab = False):
            ...
            for i in range(0, instance.shape[0]):
                ...
                if specific and limit > 0: # be more specific but limit 10 masks (avoiding lag)
                    ...
                    tt_col_specific[instance[i] == 1] = pallete[i]
                    if vocab == True:
                        self.vis.add_points(f'single object mask: ' + str(i) + '_' + str(label[i]) + '_' + str(conf2d[i].item())[:5], self.point, tt_col_specific, point_size=20, visible=True)                
                    else:
    ...
    def refinedviz(self, agnostic_path, specific = False, vocab = False):
        ...
        for i in range(0, instance.shape[0]):
            ...
            if specific and limit > 0: # be more specific but limit 10 masks (avoiding lag)
                ...
                tt_col_specific[instance[i] == 1] = pallete[i]
                if vocab == True:
                    self.vis.add_points(f'refined object mask: ' + str(i) + '_' + str(label[i]) + '_' + str(conf2d[i].item())[:5], self.point, tt_col_specific, point_size=20, visible=True)                
                else:
...
if __name__ == "__main__":
    ...
    agnostic_path = os.path.join('.pth')
    ## 6 Visualize final mask of stage 2 in each class
    check_singleviz = True
    output_dir = os.path.join("/home/zhenghao/sem2/ORganizAR/aggregation/backprojected_3d_masks.pth")
    ## 7 Visualize final refined masks
    check_refinedviz = True
    refined_path = os.path.join("/home/zhenghao/sem2/ORganizAR/aggregation/aggregated_3d_masks.pth")
    ...
    point, color = read_pointcloud(os.path.join("/home/zhenghao/sem2/ORganizAR/aggregation/downsampled_pcd.ply"))
    color = color * 127.5
    print(len(color))
    ...
    if check_singleviz:
        VIZ.singleviz(output_dir, specific = True, vocab = True)
    if check_refinedviz:
