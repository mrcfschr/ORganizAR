#------------------------------------------------------------------------------
# This script pairs LT Depth with PV frames, all previously recorded.
# Visualized as frames and as temporalily and spatially aligned frames and as colored point cloud.
# Press space to explore the pointcloud frame from different perspectives while on the o3d visualizer, press space again to continue stream.
...
path = '/Users/haoyangsun/Documents/ORganizAR/viewer/data'
# Directory containing the calibration data of the HoloLens
calibration_path: str = '/Users/haoyangsun/Documents/ORganizAR/calibration/rm_depth_longthrow/'
...
def print_attributes(msg,instance):
    attributes = vars(instance)
    print(msg)
    for attr_name, attr_value in attributes.items():
        print(f'{attr_name}: {attr_value}')

def print_associated_data(data,autofocus):
...
print_attributes("lt depth calibration data", calibration_lt)
...
while (True):
    if not is_paused:
        ...
        # # Display frames ----------------------------------------------------------
        print("Depth:")
        print_associated_data(data_lt,False)
        depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(data_lt.payload.depth, alpha=0.1), cv2.COLORMAP_JET)
        cv2.imshow('RM LT Depth', depth_colormap)
        print("PV:")
        print_associated_data(data_pv,True)
...
# just use ctrl c
# or add keyboard command and then Close readers ---------------------------------------------------------------
rd_pv.close()
rd_lt.close()
