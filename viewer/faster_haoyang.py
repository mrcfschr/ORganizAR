...
class PromptBoxManager:
    def __init__(self, prompts1, prompts2):
        ...
        self.snapshots = []
        self.output_folder = "./data/debug_faster5/"
    ...
    def new_frame(self, frame,box_threshold=0.3, text_threshold=0.25):
        ...
        for i in self.prompts:
            boxes, confidence, phrases = self.model_LangSAM.predict_dino(image, i, box_threshold,
                                                                         text_threshold)
            ...
            #update the state in the prompt_predict
            end_box = boxes[mask]
            end_box = end_box[mask_thresh]
            print(mask)
            print(type(mask))
            patch_images = image_patches[mask]
            patch_images = patch_images[mask_thresh]
            ...
            for ii, index in enumerate(end_prompts_indexes):
                self.prompt_predict[self.look_up_prompts[ii]] = {"exist": True,
    ...
    def output_det(self):
        ...
        if self.previous_assign is None:
            ...
            for i, box_index in enumerate(self.prompt_in_frame):
                ...
                self.prompt_predict[prompt]["image"].save(
                    self.output_folder + "frame" + str(self.prompt_predict[prompt]["frame"]) + prompt + ".png")
        else:
        ...
        self.previous_assign = self.prompt_in_frame
